...
from prompts.ran_classifier_prompt import classifier_prompt_v2
from utils.query_classifier_v2 import fetch_conversation_history
...
async def manage_memory(state: MessagesState):
    ...
    full_prompt = classifier_prompt_v2
    if state.get("thread_id"):
        history = await fetch_conversation_history(state.get("thread_id"))
        full_prompt = classifier_prompt_v2.replace("{conversation_history}", history)
