...
# Function to call the Station API, to fetch KPI data at CELL level
...
# Function to call the Station API, to fetch KPI data at SITE level
async def fetch_site_data(query_str: str) -> List[Dict]:
    full_url = f"{CONST.STATION_BASE_URL}{query_str}"
    logger.info(f"Calling API URL: {full_url}")
    headers = {
        "Authorization": f"Bearer {CONST.STATION_ACCESS_TOKEN}"
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(full_url, headers=headers) as response:
                ...
                # Parse JSON response
                data_response = await response.json()
                # For each KPI, fetch top 10 offending sites
                site_data_list = sort_site_data_by_kpi(data_response)
...
def build_cell_query_strings(input_data):
    output_data = []
    date_format = "%Y-%m-%dT%H:%M:%S.%f"
...
# Function to build query strings for Cell level data fetch, based on AOI offending records
def build_site_query_strings(input_data):
    output_data = []
    date_format = "%Y-%m-%dT%H:%M:%S.%f"
    ...
    for record in input_data:
        ...
        # Set sort (s) depending on KPI name
        for item in kpi_list:
            query = f"query?k={item}&o=site&fot=aoi&fol={fol}&g=1dy&st={st}&et={et}"
            output_data.append(query)
...
# identify threshold violations - CELL & SITE
def identify_threshold_violations(input_data: List[Dict]) -> List[Dict]:
...
# CELLS - sort and pick top 10
def sort_cell_data_by_kpi(input_data):
    """
...
# SITES - for each KPI, sort and pick top 10 offending sites
def sort_site_data_by_kpi(input_data):
    """
    Sorts a list of dictionaries by the 'kpivalue'.
    ...
    kpi_data = input_data.get("data", [])
    logger.info(f"kpi_key -->{kpi_key}, is_descending --> {is_descending}, count of site records -->{len(kpi_data)}")
    ...
    # # Filter out records without 'kpivalue'
    # kpi_data = [item for item in kpi_data if "kpivalue" in item]  
    # logger.info(f"Count of site records after removing null/0 kpivalue -->{len(kpi_data)}")
    ...
    # logger.info(f"Count of site records after removing blocks where kpivalue is missing -->{len(kpi_data)}")

    # with open(f'C:\\sohan\\projects\\DISH-genAI\\ran\\RAN-3-PM\\kpi_readout\\site_records_before_filter.json', "w") as f:
    #     json.dump(filtered_data, f, indent=4, ensure_ascii=False)     
...
# For SITES
def transform_data(data):
    site_map = defaultdict(lambda: {"kpis": defaultdict(list), "dates": set(), "kpi_count": 0})

    for item in data:
        site = item.get("site")
        kpi = item.get("kpiname")
        date = item.get("date")
        value = item.get("kpivalue")
        category = item.get("category")


        # Add KPI data
        site_map[site]["kpis"][kpi].append({"date": date, "kpivalue": value})
        # Track unique dates
        site_map[site]["dates"].add(date)
        # Count total KPI occurrences
        site_map[site]["kpi_count"] += 1
        # Add category information
        site_map[site]["category"] = category  # Assuming category is consistent per site
    ...
    with open(f'{DIR_PATH}\\final_sites_offenders.json', "w") as f:
        json.dump(output, f, indent=4, ensure_ascii=False)    

    return output[:10]


# Compute average KPIs from list of records
def compute_avg_kpi(kpi_records):
    sums = defaultdict(float)
    counts = defaultdict(int)

    for entry in kpi_records:
        for key, value in entry.items():
            if key not in ("object", "__time"):  # skip metadata fields
                sums[key] += value
                counts[key] += 1
