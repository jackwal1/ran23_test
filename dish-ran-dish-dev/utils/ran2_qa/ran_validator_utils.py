...
from prompts.ran_validator_prompt import validator_agent_instruction_v1,validator_agent_instruction_v2
from utils.get_history_from_checkpoint import get_conversation_context_for_llm
...
async def manage_memory(state: MessagesState):
    ...
    full_prompt = validator_agent_instruction_v2

    #print("thread is "+state.get("thread_id_sql_agent"))

    if state.get("thread_id_sql_agent"):
        history = await get_conversation_context_for_llm(
            state,
            n=10,
            trim_ai_messages=True,
            max_ai_length=100
        )
        full_prompt = validator_agent_instruction_v2.replace("{conversation_history}", history)
    #print(full_prompt)
    print('final state to LLM: \n')
    await display_messages(trimmed_messages)

    # Return the trimmed messages
    return [SystemMessage(content=full_prompt)] + trimmed_messages
