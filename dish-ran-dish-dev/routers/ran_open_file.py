...
ranQueryopenfile = APIRouter(prefix='/watsonx/ran', tags=['RAN - Docs Open/Download'])
...
@ranQueryopenfile.get("/ran_open_file", 
                   summary="RAN document based on s3 url ",
                   status_code=status.HTTP_200_OK, 
                   response_description= 'RAN Document - pdf or xlsx'
                   )
async def ran_open_file(s3_file_url: str = Query(..., description="S3 url ")):
    try:
