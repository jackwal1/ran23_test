...
class Agent:
    ...
    """    
    def __init__(self, model, tools, checkpointer, token_memory_limit=5000, system="", max_retries=3):
        self.system = system
    ...
    async def filter_memory(self, state: AgentState):
        ...
        # if token over limit, except for last but 3rd ToolMessages, delete all past messages
        if num_tokens > self.token_memory_limit:
            logger.info(f"We are going to trim the messages")

            try: 
                # Find indices of all 'ToolMessage' entries
                tm_indices = [i for i, msg in enumerate(messages) if isinstance(messages[i], ToolMessage)]
                # logger.info(f"tm_indices: {tm_indices}")
                
                trim_messages = []
                delete_messages = []
                # Make sure there are at least 2 TMs
                if len(tm_indices) >= 2:
                    second_last_tm_index = tm_indices[-2]  # Index of the second-last 'ToolMessage'
                    # go back two indexes to get HumanMessage
                    trim_messages = messages[:(second_last_tm_index-2)] 

                    # logger.info(f"delete_messages: {trim_messages}")
                ...
                logger.error(f"Error during trim messages: {e}")            
