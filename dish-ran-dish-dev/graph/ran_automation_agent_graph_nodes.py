...
from utils.ran_automation_agent_utils.automation_agent_utils import manage_memory, display_messages
...
from prompts.ran_automation_prompt import prompt
...
class Agent:
    def __init__(self, model, tools, checkpointer, token_memory_limit=100000, system= prompt, max_retries=3):
        self.system = system
    ...
    async def filter_memory(self, state: AgentState):
        """Basic memory filtering without summarization - just token checking and logging"""
        messages = state["messages"]

        # Create string of history to check tokens
        history_str = ""
        if self.system:
            history_str = "System: " + self.system + "\n"
        for msg in messages:
        ...
        # Log if over token limit but don't take action (summarization removed)
        if num_tokens > self.token_memory_limit:
            logger.warning(f"Token limit exceeded ({num_tokens} > {self.token_memory_limit}), but summarization is disabled")
...
async def initialize_agent(max_retries=3):
    ...
    try:
        ...
        langgraph_agent = Agent(
            ...
            token_memory_limit=CONST.MAX_TOKEN_THRESHOLD,
            max_retries=max_retries
        )
